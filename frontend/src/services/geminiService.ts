...
Используй живые примеры из рациона пользователя.

Пункты анализа:
1. Частота и время приемов пищи (пищевое окно, когда человек ест чаще всего, есть ли еда на ночь).
2. Степень обработки (расскажи про вредную "заводскую" еду NOVA 4, какие продукты стоит заменить на натуральные).
3. Разнообразие рациона (сколько разных продуктов, каких групп еды (овощи, фрукты и т.д.) не хватает).
4. Клетчатка (просто объясни, хватает ли ее (норма 30г) и из каких овощей/злаков ее лучше добрать).
5. Сахар и нагрузка (много ли сладкого и "быстрых" углеводов, были ли резкие скачки сахара).
6. Витамины и минералы (чего скорее всего не хватает: Железо, Магний, Витамин D, Цинк, B12).

Верни JSON объект со строковыми полями: timing, novaGroups, diversity, fiber, glycemic, deficiencies.`;
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        ...
        responseSchema: {
          type: Type.OBJECT,
          required: ["timing", "novaGroups", "diversity", "fiber", "glycemic", "deficiencies"],
          properties: {
            timing: { type: Type.STRING },
            novaGroups: { type: Type.STRING },
            diversity: { type: Type.STRING },
            fiber: { type: Type.STRING },
            glycemic: { type: Type.STRING },
    ...
    return {
      ...
      novaGroups: "Данные о степени обработки недоступны.",
      diversity: "Анализ разнообразия не выполнен.",
      fiber: "Информация о клетчатке не загружена.",
      glycemic: "Гликемический анализ не удался.",
      deficiencies: "Данные о дефицитах микронутриентов отсутствуют."
    };
  }
};
